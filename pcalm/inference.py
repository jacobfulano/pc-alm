...
def supervised_loss(params: Params, scales, skips, x, y, free, phi) -> torch.Tensor:
    y_pred = block_pred(
        params[-1], scales[-1], skips[-1], free[-1], phi, is_first=False
    )
...
def free_init(params: Params, scales, skips, x, phi) -> list[torch.Tensor]:
    return [
        ...
        for z in forward(params, scales, skips, x, phi)[:-1]
    ]


def constraint_residuals(
    params: Params, scales, skips, x, free, phi
) -> list[torch.Tensor]:
    """Return hidden model-edge constraints; the label residual is not constrained."""
    residuals = []
...
def al_energy_shifted(
    params: Params, scales, skips, x, y, free, duals, rho: float, phi
) -> torch.Tensor:
...
def run_pc(
    params: Params, scales, skips, x, y, *, state_lr: float, rho: float, steps: int, phi
):
    inference_params = [param.detach() for param in params]
    free0 = free_init(inference_params, scales, skips, x, phi)
    duals0 = zero_duals_like(
        constraint_residuals(inference_params, scales, skips, x, free0, phi)
    )
    free = _solve_inner(
        inference_params, scales, skips, x, y, free0, duals0, state_lr, rho, steps, phi
    )
    return [z.detach() for z in free], duals0
...
def run_pcalm(
    ...
    *,
    ...
    if weight_credit_timing not in {"pre_dual_energy", "post_dual_energy"}:
        raise ValueError(
            "weight_credit_timing must be pre_dual_energy or post_dual_energy"
        )

    inference_params = [param.detach() for param in params]
    free = free_init(inference_params, scales, skips, x, phi)
    duals = zero_duals_like(
        constraint_residuals(inference_params, scales, skips, x, free, phi)
    )
    ...
    for _ in range(budget - 1):
        ...
        residuals = constraint_residuals(inference_params, scales, skips, x, free, phi)
        duals = [
    ...
    duals_weight = duals if weight_credit_timing == "pre_dual_energy" else duals_after
    return [z.detach() for z in free], [dual.detach() for dual in duals_weight]


def infer_for_schedule(
    params: Params,
    scales,
    skips,
    x,
...
def method_grad(
    ...
    *,
    ...
    if schedule.family == "bp":
        return list(
            torch.autograd.grad(bp_loss(params, scales, skips, x, y, phi), params)
        )
    free, duals = infer_for_schedule(
        params, scales, skips, x, y, schedule, state_lr=state_lr, rho=rho, phi=phi
    )
    energy = al_energy_shifted(params, scales, skips, x, y, free, duals, rho, phi)
    return list(torch.autograd.grad(energy, params))
...
def _solve_inner(
    ...
    for _ in range(max(steps, 0)):
        energy = al_energy_shifted(params, scales, skips, x, y, free, duals, rho, phi)
        grads = torch.autograd.grad(energy, free)
        free = [
            (z - effective_lr * grad).detach().requires_grad_(True)
            for z, grad in zip(free, grads, strict=True)
        ]
    return free
